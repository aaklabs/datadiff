...
public class CompareService {
	...
	public CompareResponse compareSqlQueryAndResponseAsSingleGrid(String db1Sql, String db2Sql , DBSource db1Source, DBSource db2Source) throws Exception {
		...
		for (RowCompareResult columns : response) {
			...
			for (ColumnCompareResult column : columns.getColumns()) {
				GridRowColumn columnDb1 = new GridRowColumn(column.getDb1().getRowNo(), column.getDb1().getColumnName(), column.getDb1().getColumnValue(), column.getDb1().getDataType(), column.isDifferent(), column.isOnlyInDb1(), column.isOnlyInDb2(), column.isNullInBoth(), column.getComments(), column.isError());
				GridRowColumn columnDb2 = new GridRowColumn(column.getDb2().getRowNo(), column.getDb2().getColumnName(), column.getDb2().getColumnValue(), column.getDb2().getDataType(), column.isDifferent(), column.isOnlyInDb1(), column.isOnlyInDb2(), column.isNullInBoth(), column.getComments(), column.isError());
