...
public class CompareManager {
	...
	public List<RowCompareResult> compareItem(List<DBRow> db1Rows, List<DBRow> db2Rows) {
		...
		for(int i=0; i < db1RowCount; i++) {
			DBRow db1RowRecord = db1Rows.get(i);
			DBRow db2RowRecord = db2Rows.get(i);
			...
			if(db1RowItemColumnCount != db2RowItemColumnCount) {
				log.error("Column count not maching for db1RowRecord ->{} and db2RowRecord ->{}",db1RowItemColumnCount,db2RowItemColumnCount);
			}
			...
			for (int j = 0; j < db1RowItemColumnCount; j++) {
				...
				DBColumn db1RowColRecord = db1RowRecord.getColumns().get(j);
				DBColumn db2RowColRecord = db2RowRecord.getColumns().get(j);
